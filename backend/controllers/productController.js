...
const searchProductbyCategory = async (req, res) => {
    try {
        ...
        let products = await Product.find({
            $or: [
                { category: { $regex: `.*${key.split('').join('.*')}.*`, $options: 'i' } },
            ]
        }).populate("seller", "shopName");
...
//new.....
const countTotalProductsAddedByCustomers = async (req, res) => {
    try {
        const sellerId = req.params.id;
        ...
        customersWithSellerProduct.forEach(customer => {
            customer.cartDetails.forEach(cartItem => {
                if (cartItem.seller.toString() === sellerId) {
                    totalProducts += cartItem.quantity; // Increment total products by quantity
                }
            });
        });
